...
- 多域名资产录入与管理
- 域名概览页按“接入与治理总览 -> 承载排行 -> 资产结构 -> Cloudflare 快照”组织
- 概览页保留固定高度内部滚动，适合运营驾驶舱式浏览
...
- 域名维度监控卡片、排行图表、接入概览
- 卡片、图表、辅助文字统一接入主题语义，亮色 / 暗色保持一致的状态表达
...
- 通知端点支持自定义请求头，可补充 `Authorization`、`X-App-Key` 等 webhook header
- 测试投递支持选择事件并发送自定义 JSON 负载，便于联调签名和下游解析
...
当前概览页已经按运营驾驶舱思路重排为固定高度内滚动布局：先看接入覆盖、健康覆盖、托管率和治理标签，再看域名承载排行，最后下钻资产结构与 Cloudflare 快照。卡片、图表和辅助文字都复用同一套主题语义组件，亮色 / 暗色下保持一致的状态关系。
