...
export default function AuditLogsPage({ currentUser, onUnauthorized }: AuditLogsPageProps) {
  ...
  async function loadData(paramsSource: URLSearchParams = searchParams) {
    ...
    try {
      ...
      const payload = await getAuditLogs(getCurrentPage(params.get("page")), {
        action_prefix: normalizeSearchValue(params.get("action_prefix")),
        entity_type: normalizeSearchValue(params.get("entity_type")),
